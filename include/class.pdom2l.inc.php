...
class PdoM2l
{
    ...
    private static $bdd = 'dbname=GuilM2L1402';
    ...
    {
        $req = "select departement.code as codeDep, departement.nom as nomDep, count(ligue.id) as nbLigue from departement left JOIN ligue ON
                departement.code = ligue.codeDep group by departement.code";
    ...
    public function getInfosLigue($idLigue){
        $req = "select ligue.id as idLigue, ligue.nom as nomLigue, ligue.adresseRue as adrLigue, ligue.cp as cpLigue, 
                ligue.ville as villeLigue, ligue.emailContact as emailLigue, ligue.urlSiteWeb as urlLigue, ligue.tel as telLigue, 
                ligue.logo as logoLigue, disciplines.olympique as discipLigue
		        from  ligue, disciplines where ligue.id ='$idLigue' and ligue.idDisciplines = disciplines.id";
